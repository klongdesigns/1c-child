...
echo '<link rel="stylesheet" type="text/css" href="' . get_bloginfo('stylesheet_directory') . '/1call/custom_admin.css" />';
}
...
function new_wp_login_title() {
    return get_option('blogname') . ' - Powered by 1 Call Service';
}
...
/**********************************************************************************
	...
	If also hides the updates notifications for any user that is NOT 1call (our user)
**********************************************************************************/
function custom_admin_css() {
	global $current_user;
	get_currentuserinfo();
echo '<link rel="stylesheet" id="custom_admin" type="text/css" href="' . get_bloginfo('stylesheet_directory') . '/1call/custom_admin.css" />';
	if ($current_user->user_login != '1call') {
...
Website designed and maintained by <a href="http://www.1callservice.com/" target="_blank">1 Call Service</a> | <a href="http://www.1callservice.com/support" target="_blank">Support</a>
<?php }
...
wp_add_dashboard_widget( 'tutorial_dashboard', __( 'Using Your 1 Call Service CMS' ), 'tutorial_dashboard' );
}
...
/**********************************************************************************
	...
	This function removes certain panels that aren't used from non 1call users.
**********************************************************************************/

function onecall_lock() {
	global $userdata;
	get_currentuserinfo();
	if ($userdata->user_login != '1call') {
...
add_action( 'admin_menu', 'onecall_lock');
add_action( 'admin_init', 'onecall_lock_theme');